...
import { createReceiverForSender, isDisabled } from '../../../../farm';

// Redux
import { useSelector } from 'react-redux';
import { selectCustomerInfo } from '../../../state/reducers/store';

function CreateReceiverScreen({ route, navigation }) {
  ...
  const handleReceiverCreation = async () => {
    setIsLoading(true);
    try {
      await createReceiverForSender({
        name: name.trim(),
        surname: surname.trim(),
        number: number.trim(),
        senderId,
        ownerSub: customer?.sub,
      });
      Alert.alert('Success', 'Receiver created successfully!');
      navigation.navigate('Customers');
    } catch (error) {
      Alert.alert('Error', 'Failed to create receiver. Please try again.');
    }
    setIsLoading(false);
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          <View style={styles.iconContainer}>
            <MaterialCommunityIcons name="account-plus-outline" size={40} color={appColor.secondary} />
          </View>
          <Text style={styles.title}>New Receiver</Text>
          <Text style={styles.subtitle}>Add a new recipient for this customer</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <TouchableOpacity
            ...
            onPress={handleReceiverCreation}
          >
            {isLoading ? (
              ...
              <>
                ...
                <Text style={styles.primaryButtonText}>Create Receiver</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
